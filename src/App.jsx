...
function getSemenRecordInFinancialPeriod(record, fyStartYear, monthLabel = "Overall") {
  const dates = getSemenStockDateCandidates(record);
  if (!dates.length) return monthLabel === "Overall";
  return dates.some((dt) => {
    const fy = dt.getMonth() >= 3 ? dt.getFullYear() : dt.getFullYear() - 1;
    if (fy !== fyStartYear) return false;
    if (monthLabel === "Overall") return true;
    const info = getFinancialMonthYear(fyStartYear, monthLabel);
    return info && dt.getFullYear() === info.year && dt.getMonth() === info.monthIndex;
  });
}
...
function buildSemenStockReportRows(animals, fyStartYear, monthLabel = "Overall") {
  ...
  (animals || [])
    ...
    .sort(sortByTag)
    .forEach((bull) => {
      (bull?.maleDetails?.semenStockRecords || []).map(cleanSemenStockRecord).forEach((record, idx) => {
        if (!getSemenRecordInFinancialPeriod(record, fyStartYear, monthLabel)) return;
        rows.push({
          bullTag: bull.tagNo || "",
          setNo: bull.breedingSet || "",
          status: isArchivedAnimal(bull) ? "Archived" : "Current",
          rowNo: idx + 1,
          openingBalance: numberOrZero(record.openingBalance),
          dosesProduced: numberOrZero(record.dosesProduced),
          productionDate: record.productionDate || "",
          dosesSold: numberOrZero(record.dosesSold),
          saleDate: record.saleDate || "",
          dosesIssuedFarm: numberOrZero(record.dosesIssuedFarm),
          farmIssueDate: record.farmIssueDate || "",
          dosesIssuedGadvasu: numberOrZero(record.dosesIssuedGadvasu),
          gadvasuIssueDate: record.gadvasuIssueDate || "",
          dosesUsedExperiments: numberOrZero(record.dosesUsedExperiments),
          experimentDate: record.experimentDate || "",
          closingBalance: calculateSemenClosingBalance(record),
        });
      });
    });
  return rows;
