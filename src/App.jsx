...
export default function AnimalDataRecordingApp() {
  ...
  function exportDairyScopeReport(reportType) {
    ...
    if (reportType === "semenStock") {
      ...
      autoTable(doc, {
        startY: 42,
        head: [["Bull", "Set", "Status", "Opening", "Produced", "Prod. Date", "Sold", "Sale Date", "Farm Issue", "Farm Date", "GADVASU Issue", "GADVASU Date", "Experiment", "Exp. Date", "Closing"]],
        body: semenStockReportRows.length ? semenStockReportRows.map((row) => [
          row.bullTag, row.setNo, row.status, row.openingBalance, row.dosesProduced, row.productionDate, row.dosesSold, row.saleDate, row.dosesIssuedFarm, row.farmIssueDate, row.dosesIssuedGadvasu, row.gadvasuIssueDate, row.dosesUsedExperiments, row.experimentDate, row.closingBalance,
        ]) : [["No records", "", "", "", "", "", "", "", "", "", "", "", "", "", ""]],
                ...
                <div className="rounded-[28px] border border-emerald-100 bg-white/95 p-5 shadow-[0_12px_28px_rgba(15,23,42,0.06)]">
                  <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                    <div>
                      ...
                      <div className="mt-1 text-sm text-slate-600">Select financial year and month to preview bull-wise semen stock on screen, then export the same report as PDF.</div>
                    </div>
                      ...
                      <tbody>
                        {semenStockReportRows.length ? semenStockReportRows.map((row, idx) => (
                          <tr key={`semen-report-${row.bullTag}-${idx}`} className="align-top">
                            ...
                            <td className="border border-emerald-100 px-2 py-2 text-right"><div>{row.dosesProduced}</div><div className="text-[10px] text-slate-500">{row.productionDate || ""}</div></td>
                            <td className="border border-emerald-100 px-2 py-2 text-right"><div>{row.dosesSold}</div><div className="text-[10px] text-slate-500">{row.saleDate || ""}</div></td>
                            <td className="border border-emerald-100 px-2 py-2 text-right"><div>{row.dosesIssuedFarm}</div><div className="text-[10px] text-slate-500">{row.farmIssueDate || ""}</div></td>
                            <td className="border border-emerald-100 px-2 py-2 text-right"><div>{row.dosesIssuedGadvasu}</div><div className="text-[10px] text-slate-500">{row.gadvasuIssueDate || ""}</div></td>
                            <td className="border border-emerald-100 px-2 py-2 text-right"><div>{row.dosesUsedExperiments}</div><div className="text-[10px] text-slate-500">{row.experimentDate || ""}</div></td>
