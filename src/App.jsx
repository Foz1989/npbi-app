...
import highlight1Image from "./highlight-1.png";
import highlight2Image from "./highlight-2.png";
import highlight3Image from "./highlight-3.png";
import highlight4Image from "./highlight-4.png";
import highlight5Image from "./highlight-5.png";
import highlight6Image from "./highlight-6.png";
...
function getStandardMonthlyHerdAgeCategories() {
  return [
    { label: "0-30 days", test: (days) => days !== null && days >= 0 && days <= 30 },
    { label: "31-60 days", test: (days) => days !== null && days >= 31 && days <= 60 },
    { label: "61-90 days", test: (days) => days !== null && days >= 61 && days <= 90 },
    { label: "91-180 days", test: (days) => days !== null && days >= 91 && days <= 180 },
    { label: "181-365/366 days", test: (days) => days !== null && days >= 181 && days <= 366 },
    { label: "12-24 months", test: (days, months) => days !== null && days > 366 && months !== null && months < 24 },
    { label: "24-30 months", test: (days, months) => months !== null && months >= 24 && months < 30 },
    { label: "30-36 months", test: (days, months) => months !== null && months >= 30 && months < 36 },
    { label: ">36 months", test: (days, months) => months !== null && months >= 36 },
  ];
}

function getStandardMonthlyHerdAgeLabel(animal, date) {
  const days = ageDaysOnDate(animal, date);
  const months = ageMonthsOnDate(animal, date);
  const matched = getStandardMonthlyHerdAgeCategories().find((cat) => cat.test(days, months));
  return matched?.label || "";
}

function isBreedingBullBySet(animal) {
  return animal?.category === "Male" && Boolean(String(animal?.breedingSet || "").trim());
}

function getFunctionalHerdCategory(animal, date) {
  if (!animal || !date) return "";
  if (animal.category === "Female") {
    const lifecycle = getFemaleLifecycleOnDate(animal, formatDateDisplay(date));
    if (lifecycle === "Milk") return "Lactating Buffaloes";
    if (lifecycle === "Dry" || lifecycle === "Colostrum") return "Dry Buffaloes";
    return "";
  }
  if (isBreedingBullBySet(animal)) return "Breeding Bulls";
  return "";
}

function getFunctionalHerdRowsFromPresent(present, asOfDate) {
  const lactating = present.filter((a) => getFunctionalHerdCategory(a, asOfDate) === "Lactating Buffaloes").length;
  const dry = present.filter((a) => getFunctionalHerdCategory(a, asOfDate) === "Dry Buffaloes").length;
  const breedingBulls = present.filter((a) => getFunctionalHerdCategory(a, asOfDate) === "Breeding Bulls").length;
  return [
    { label: "Lactating Buffaloes", male: 0, female: lactating, total: lactating },
    { label: "Dry Buffaloes", male: 0, female: dry, total: dry },
    { label: "Breeding Bulls", male: breedingBulls, female: 0, total: breedingBulls },
  ];
}
...
function buildHerdStrengthReport(animals, fyStartYear, monthLabel) {
  ...
  const categories = getStandardMonthlyHerdAgeCategories();

  const femaleRows = categories.map(({ label, test }) => {
    const female = present.filter((a) => a.category === "Female" && test(ageDaysOnDate(a, asOfDate), ageMonthsOnDate(a, asOfDate))).length;
    return makeRow(label, 0, female);
  });

  const maleRows = categories.map(({ label, test }) => {
    const male = present.filter((a) => a.category === "Male" && test(ageDaysOnDate(a, asOfDate), ageMonthsOnDate(a, asOfDate))).length;
    return makeRow(label, male, 0);
  });

  const functionalRows = getFunctionalHerdRowsFromPresent(present, asOfDate);
  ...
  return {
    ...
    maleRows,
    functionalRows,
    femaleTotals: makeRow("Total", 0, total(femaleRows, "female")),
    maleTotals: makeRow("Total", total(maleRows, "male"), 0),
    functionalTotals: makeRow("Total", total(functionalRows, "male"), total(functionalRows, "female")),
  };
}
...
function getMovementCategory(animal, date) {
  if (!animal || !date) return "";
  const sexKey = animal.category === "Female" ? "F" : animal.category === "Male" ? "M" : "";
  if (!sexKey) return "";
  const label = getStandardMonthlyHerdAgeLabel(animal, date);
  return label ? `${sexKey}:${label}` : "";
}

function createMovementRows(section) {
  const labels = getStandardMonthlyHerdAgeCategories().map((cat) => cat.label);
...
function createFunctionalMovementRows() {
  return ["Lactating Buffaloes", "Dry Buffaloes", "Breeding Bulls"].map((label) => ({
    key: `Functional:${label}`,
    label,
    ob: 0,
    b: 0,
    p: 0,
...
function getFunctionalMovementKey(animal, date) {
  const label = getFunctionalHerdCategory(animal, date);
  return label ? `Functional:${label}` : "";
}
...
function buildMonthlyHerdMovementReport(animals, fyStartYear, monthLabel) {
  ...
  const functionalRows = createFunctionalMovementRows();
  const byKey = Object.fromEntries([...rows, ...functionalRows].map((row) => [row.key, row]));
  ...
  (animals || []).forEach((animal) => {
    ...
    const startFunctionalCategory = presentBeforeStart ? getFunctionalMovementKey(animal, beforeStart) : "";
    const endFunctionalCategory = presentEnd ? getFunctionalMovementKey(animal, endDate) : "";

    if (startCategory) add(startCategory, "ob");
    if (endCategory) add(endCategory, "cb");
    if (startFunctionalCategory) add(startFunctionalCategory, "ob");
    if (endFunctionalCategory) add(endFunctionalCategory, "cb");

    if (bornDuringMonth) {
      const birthCategory = getMovementCategory(animal, dob);
      const birthFunctionalCategory = getFunctionalMovementKey(animal, dob);
      add(birthCategory, "b");
      if (birthFunctionalCategory) add(birthFunctionalCategory, "b");
    }

    if (purchasedDuringMonth) {
      const purchaseCategory = getMovementCategory(animal, entryDate);
      const purchaseFunctionalCategory = getFunctionalMovementKey(animal, entryDate);
      add(purchaseCategory, "p");
      if (purchaseFunctionalCategory) add(purchaseFunctionalCategory, "p");
    }
    ...
    if (exitedDuringMonth) {
      ...
      const exitFunctionalCategory = getFunctionalMovementKey(animal, exitReferenceDate) || getFunctionalMovementKey(animal, exitDate);
      const type = getMovementExitType(animal);
      if (type === "D") {
        add(exitCategory, "d");
        if (exitFunctionalCategory) add(exitFunctionalCategory, "d");
      } else if (type === "TOut") {
        add(exitCategory, "tout");
        if (exitFunctionalCategory) add(exitFunctionalCategory, "tout");
      } else {
        add(exitCategory, "s");
        if (exitFunctionalCategory) add(exitFunctionalCategory, "s");
      }
    }
    ...
    if (presentBeforeStart && presentEnd && !bornDuringMonth && !exitedDuringMonth && startFunctionalCategory !== endFunctionalCategory) {
      if (startFunctionalCategory) add(startFunctionalCategory, "tout");
      if (endFunctionalCategory) add(endFunctionalCategory, "tin");
    }
  });
  ...
  const functionalTotal = totalRow("Functional Sub Total", functionalRows);
  const grandTotal = totalRow("Age-wise Grand Total", [femaleTotal, maleTotal]);

  return { startDate, endDate, femaleRows, maleRows, functionalRows, femaleTotal, maleTotal, functionalTotal, grandTotal };
}

function HerdMovementTable({ report }) {
  const sections = [
    { title: "Female age-wise movement", rows: report.femaleRows, total: report.femaleTotal },
    { title: "Male age-wise movement", rows: report.maleRows, total: report.maleTotal },
    { title: "Functional status movement", rows: report.functionalRows || [], total: report.functionalTotal || { key: "Functional Sub Total", label: "Functional Sub Total", ob: 0, b: 0, p: 0, tin: 0, d: 0, s: 0, tout: 0, cb: 0 } },
  ];
              ...
              <div className="rounded-[28px] border border-white/10 bg-white/[0.04] p-4">
                <div className="flex items-center gap-3">
                  <div className="flex h-16 w-16 items-center justify-center overflow-hidden rounded-full border border-emerald-300/30 bg-white p-2 shadow-[0_12px_28px_rgba(16,185,129,0.18)]">
                    <img src={logoImage} alt="DairyScope logo" className="h-full w-full object-contain" />
                  </div>
                  <div>
              ...
              <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                <div className="flex items-center gap-4">
                  <div className="hidden h-12 w-12 items-center justify-center overflow-hidden rounded-full border border-emerald-100 bg-white p-1 shadow-sm sm:flex">
                    <img src={logoImage} alt="DairyScope" className="h-full w-full object-contain" />
                  </div>
                  <div>
                ...
                <div className="mt-7 grid gap-5 lg:grid-cols-2">
                  <div className="rounded-[28px] border border-emerald-100 bg-emerald-50/60 p-5 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-lg">
                    <div className="flex items-start gap-5">
                      <img src={highlight1Image} alt="Herd Intelligence" className="ml-3 h-28 w-40 flex-shrink-0 object-contain object-center" />
                      <div>
                  ...
                  <div className="rounded-[28px] border border-cyan-100 bg-cyan-50/60 p-5 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-lg">
                    <div className="flex items-start gap-5">
                      <img src={highlight2Image} alt="Genetic and Breeding Engine" className="ml-3 h-28 w-40 flex-shrink-0 object-contain object-center" />
                      <div>
                  ...
                  <div className="rounded-[28px] border border-blue-100 bg-blue-50/60 p-5 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-lg">
                    <div className="flex items-start gap-5">
                      <img src={highlight3Image} alt="Production Analytics" className="ml-3 h-28 w-40 flex-shrink-0 object-contain object-center" />
                      <div>
                  ...
                  <div className="rounded-[28px] border border-violet-100 bg-violet-50/60 p-5 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-lg">
                    <div className="flex items-start gap-5">
                      <img src={highlight4Image} alt="Reproduction Monitoring" className="ml-3 h-28 w-40 flex-shrink-0 object-contain object-center" />
                      <div>
                  ...
                  <div className="rounded-[28px] border border-amber-100 bg-amber-50/60 p-5 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-lg">
                    <div className="flex items-start gap-5">
                      <img src={highlight5Image} alt="Bull Evaluation System" className="ml-3 h-28 w-40 flex-shrink-0 object-contain object-center" />
                      <div>
                  ...
                  <div className="rounded-[28px] border border-slate-200 bg-slate-50/80 p-5 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-lg">
                    <div className="flex items-start gap-5">
                      <img src={highlight6Image} alt="Animal Documentation" className="ml-3 h-28 w-40 flex-shrink-0 object-contain object-center" />
                      <div>
          ...
          <>
            ...
            <Section title="Monthly Herd Strength Report">
              ...
              <div className="space-y-5">
                ...
                <HerdStrengthTable title="Females herd strength" rows={herdStructureReport.femaleRows} totalRow={herdStructureReport.femaleTotals} />

                <HerdStrengthTable title="Males herd strength" rows={herdStructureReport.maleRows} totalRow={herdStructureReport.maleTotals} />

                <HerdStrengthTable title="Functional herd strength" rows={herdStructureReport.functionalRows || []} totalRow={herdStructureReport.functionalTotals || { label: "Total", male: 0, female: 0, total: 0 }} />

                <div className="rounded-[24px] border border-slate-100 bg-white/70 px-4 py-4">
                  <div className="text-xl font-bold text-slate-950">Total Herd = {herdStructureReport.femaleTotals.total + herdStructureReport.maleTotals.total}</div>
                </div>
              </div>
            </Section>
