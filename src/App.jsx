...
function buildFemaleHistoryPedigreeDetails(animal, allAnimals = []) {
  ...
  const sireItems = [
    ...
    return { ...item, index: getAncestorIndexValue(matched) };
  });
...
function drawFemaleHistoryHeader(doc, full, pedigreeDetails) {
  ...
  const damBody = (pedigreeDetails?.damItems || []).map((r) => {
    const by = (key) => (r.production || []).map((p) => p[key]).filter(Boolean).join(", ");
    return [r.label, r.tagNo, by("tlmy"), by("slmy"), by("ll")];
  });
  autoTable(doc, {
    startY: 39,
    styles: { fontSize: 6.5, cellPadding: 0.65, lineWidth: 0.10, minCellHeight: 5, textColor: [0, 0, 0], fontStyle: "bold", overflow: "linebreak" },
    headStyles: { fontSize: 6.5, fillColor: [220, 245, 232], textColor: [0, 0, 0], fontStyle: "bold" },
    bodyStyles: { textColor: [0, 0, 0], fontStyle: "bold" },
    theme: "grid",
    head: [["Dam side", "Tag No.", "TLMY parity-wise", "SLMY parity-wise", "LL parity-wise"]],
    body: damBody,
    margin: { left: 126, right: 6 },
    tableWidth: 165,
    columnStyles: { 0: { cellWidth: 34 }, 1: { cellWidth: 20 }, 2: { cellWidth: 42 }, 3: { cellWidth: 42 }, 4: { cellWidth: 27 } },
  });

  return Math.max(doc.lastAutoTable?.finalY || 59, 61) + 4;
}
        ...
        {(sidebarView === "add" || showAdd) && (
          <Section title={editAnimalId ? "Edit Animal" : "Add Animal"}>
            <Grid>
              ...
              <TextField label="Index" value={newAnimal.indexNo || ""} onChange={(v) => setNewAnimal((s) => ({ ...s, indexNo: v }))} />
                  ...
                  <div className="space-y-5">
                    ...
                    <div className="rounded-[24px] border border-emerald-100 bg-white p-4 shadow-sm">
                      ...
                      <div className="grid gap-4 xl:grid-cols-2">
                        ...
                        <div className="overflow-x-auto rounded-2xl border border-emerald-100">
                          <table className="min-w-full text-sm">
                            <thead className="bg-emerald-50 text-emerald-900"><tr>{["Dam side", "Tag No.", "Parity", "TLMY", "SLMY", "LL"].map((h) => <th key={h} className="border border-emerald-100 px-3 py-2 text-left">{h}</th>)}</tr></thead>
                            <tbody>{femaleHistoryPedigreeDetails.damItems.flatMap((row) => {
                              const filled = (row.production || []).filter((p) => p.tlmy || p.slmy || p.ll);
                              const rowsToShow = filled.length ? filled : [{ parity: "", tlmy: "", slmy: "", ll: "" }];
                              return rowsToShow.map((p, idx) => <tr key={`${row.label}-${idx}`}><td className="border border-emerald-100 px-3 py-2 font-semibold">{idx === 0 ? row.label : ""}</td><td className="border border-emerald-100 px-3 py-2">{idx === 0 ? row.tagNo : ""}</td><td className="border border-emerald-100 px-3 py-2">{p.parity}</td><td className="border border-emerald-100 px-3 py-2">{p.tlmy}</td><td className="border border-emerald-100 px-3 py-2">{p.slmy}</td><td className="border border-emerald-100 px-3 py-2">{p.ll}</td></tr>);
                            })}</tbody>
                          </table>
                        </div>
                      </div>
