...
import DairyScopeLogo from "./logo.png";
import Highlight1 from "./highlight-1.png";
import Highlight2 from "./highlight-2.png";
import Highlight3 from "./highlight-3.png";
import Highlight4 from "./highlight-4.png";
import Highlight5 from "./highlight-5.png";
import Highlight6 from "./highlight-6.png";
...
const MALE_TABS = ["photographs", "pedigree", "disease testing", "dam record", "ai information", "progenies born", "performance of daughters", "health", "semen stock", "overall history sheet"];
...
function makeSemenStockRecord(openingBalance = "") {
  return {
    openingBalance: openingBalance === undefined || openingBalance === null ? "" : String(openingBalance),
    dosesProduced: "",
    productionDate: "",
    dosesSold: "",
    saleDate: "",
...
function calculateSemenClosingBalance(record) {
  return numberOrZero(record?.openingBalance)
    + numberOrZero(record?.dosesProduced)
    - numberOrZero(record?.dosesSold)
    - numberOrZero(record?.dosesIssuedFarm)
    - numberOrZero(record?.dosesIssuedGadvasu)
    - numberOrZero(record?.dosesUsedExperiments);
}

function cleanSemenStockRecord(record = {}) {
  return {
    ...makeSemenStockRecord(record.openingBalance),
    ...record,
  };
}
...
const emptyMaleDetails = {
  ...
  semenStockRecords: [makeSemenStockRecord()],
  health: { ...emptyHealth },
  historyMeta: {
    remarks: "",
    bookValue: "",
...
function withDefaults(animal) {
  ...
  const maleDetails = {
    ...
    semenStockRecords: animal.maleDetails?.semenStockRecords?.length ? animal.maleDetails.semenStockRecords.map((r) => cleanSemenStockRecord(r)) : [makeSemenStockRecord()],
...
function getFinancialYearOptions(animals) {
  ...
  (animals || []).forEach((a) => {
    ...
    (a?.maleDetails?.semenStockRecords || []).forEach((r) => {
      dates.push(r.productionDate, r.saleDate, r.farmIssueDate, r.gadvasuIssueDate, r.experimentDate);
    });
    dates.forEach((d) => {
...
function getSemenStockDateCandidates(record) {
  return [record?.productionDate, record?.saleDate, record?.farmIssueDate, record?.gadvasuIssueDate, record?.experimentDate]
    .map((d) => parseDisplayDate(d || ""))
    .filter(Boolean);
}

function getSemenFinancialPeriodBounds(fyStartYear, monthLabel = "Overall") {
  if (monthLabel === "Overall") {
    return {
  ...
  if (!info) return null;
  return {
    start: new Date(info.year, info.monthIndex, 1),
    end: new Date(info.year, info.monthIndex + 1, 0, 23, 59, 59, 999),
  };
}

function isDateInSemenPeriod(dateStr, period) {
  const dt = parseDisplayDate(dateStr || "");
  if (!dt || !period) return false;
  return dt.getTime() >= period.start.getTime() && dt.getTime() <= period.end.getTime();
}

function isDateBeforeSemenPeriod(dateStr, period) {
  const dt = parseDisplayDate(dateStr || "");
  if (!dt || !period) return false;
  return dt.getTime() < period.start.getTime();
}

function joinSemenDates(dates) {
  return [...new Set((dates || []).filter(Boolean))].join(", ");
}

function buildSemenStockReportRows(animals, fyStartYear, monthLabel = "Overall") {
  const period = getSemenFinancialPeriodBounds(fyStartYear, monthLabel);
  if (!period) return [];
  const rows = [];

  (animals || [])
    .filter((animal) => animal?.category === "Male" && animal?.isBreedingBull === "Yes")
    .sort(sortByTag)
    .forEach((bull) => {
      const records = (bull?.maleDetails?.semenStockRecords || []).map(cleanSemenStockRecord);
      if (!records.length) return;

      const firstOpeningRecord = records.find((record) => String(record.openingBalance || "").trim() !== "") || records[0];
      const baseOpening = numberOrZero(firstOpeningRecord?.openingBalance);
      let beforeProduced = 0;
      let beforeSold = 0;
      let beforeFarm = 0;
      let beforeGadvasu = 0;
      let beforeExperiment = 0;
      let produced = 0;
      let sold = 0;
      let farm = 0;
      let gadvasu = 0;
      ...
      records.forEach((record) => {
        if (isDateBeforeSemenPeriod(record.productionDate, period)) beforeProduced += numberOrZero(record.dosesProduced);
        if (isDateBeforeSemenPeriod(record.saleDate, period)) beforeSold += numberOrZero(record.dosesSold);
        if (isDateBeforeSemenPeriod(record.farmIssueDate, period)) beforeFarm += numberOrZero(record.dosesIssuedFarm);
        if (isDateBeforeSemenPeriod(record.gadvasuIssueDate, period)) beforeGadvasu += numberOrZero(record.dosesIssuedGadvasu);
        if (isDateBeforeSemenPeriod(record.experimentDate, period)) beforeExperiment += numberOrZero(record.dosesUsedExperiments);

        if (isDateInSemenPeriod(record.productionDate, period)) { produced += numberOrZero(record.dosesProduced); producedDates.push(record.productionDate); }
        if (isDateInSemenPeriod(record.saleDate, period)) { sold += numberOrZero(record.dosesSold); soldDates.push(record.saleDate); }
        if (isDateInSemenPeriod(record.farmIssueDate, period)) { farm += numberOrZero(record.dosesIssuedFarm); farmDates.push(record.farmIssueDate); }
        if (isDateInSemenPeriod(record.gadvasuIssueDate, period)) { gadvasu += numberOrZero(record.dosesIssuedGadvasu); gadvasuDates.push(record.gadvasuIssueDate); }
        if (isDateInSemenPeriod(record.experimentDate, period)) { experiment += numberOrZero(record.dosesUsedExperiments); experimentDates.push(record.experimentDate); }
      });

      const openingBalance = baseOpening + beforeProduced - beforeSold - beforeFarm - beforeGadvasu - beforeExperiment;
      const closingBalance = openingBalance + produced - sold - farm - gadvasu - experiment;
      const hasPeriodMovement = produced || sold || farm || gadvasu || experiment;
      const hasStockRecord = records.some((record) => String(record.openingBalance || record.dosesProduced || record.dosesSold || record.dosesIssuedFarm || record.dosesIssuedGadvasu || record.dosesUsedExperiments || "").trim() !== "");
      if (!hasStockRecord && !hasPeriodMovement && !openingBalance && !closingBalance) return;

      rows.push({
        bullTag: bull.tagNo || "",
        setNo: bull.breedingSet || "",
        status: isArchivedAnimal(bull) ? "Archived" : "Current",
        openingBalance,
        dosesProduced: produced,
        productionDate: joinSemenDates(producedDates),
        dosesSold: sold,
        saleDate: joinSemenDates(soldDates),
        dosesIssuedFarm: farm,
        farmIssueDate: joinSemenDates(farmDates),
        dosesIssuedGadvasu: gadvasu,
        gadvasuIssueDate: joinSemenDates(gadvasuDates),
        dosesUsedExperiments: experiment,
        experimentDate: joinSemenDates(experimentDates),
        closingBalance,
      });
    });
  return rows;
...
function drawFemaleHistoryHeader(doc, full, pedigreeDetails) {
  ...
  (pedigreeDetails?.damItems || []).forEach((r) => {
    ...
    const title = `${r.label} — Tag No.: ${r.tagNo || ""}`;
    doc.setFontSize(7.4);
...
function DateInput({ value, onChange }) {
  return (
    <input
      className="w-full rounded-xl border border-emerald-100 px-2 py-2"
      value={value}
      placeholder="dd/mm/yyyy"
      onChange={(e) => onChange(normalizeDisplayDate(e.target.value))}
    />
  );
}
...
export default function AnimalDataRecordingApp() {
  ...
  const [selectedSemenStockMonth, setSelectedSemenStockMonth] = useState("Overall");
  ...
  const semenStockReportRows = useMemo(() => buildSemenStockReportRows(normalizedAnimals, selectedFinancialYearNum, selectedSemenStockMonth), [normalizedAnimals, selectedFinancialYearNum, selectedSemenStockMonth]);
  ...
  function exportDairyScopeReport(reportType) {
    ...
    if (reportType === "semenStock") {
      const fyLabel = financialYearOptions.find((o) => String(o.value) === String(selectedFinancialYear))?.label || String(selectedFinancialYear || selectedFinancialYearNum);
      const monthLabel = selectedSemenStockMonth === "Overall" ? "Whole Financial Year" : selectedSemenStockMonth;
      header("SEMEN STOCK REPORT", `Financial Year: ${fyLabel} · Period: ${monthLabel}`);
      const total = semenStockReportRows.reduce((acc, row) => ({
        openingBalance: acc.openingBalance + row.openingBalance,
        dosesProduced: acc.dosesProduced + row.dosesProduced,
        dosesSold: acc.dosesSold + row.dosesSold,
        dosesIssuedFarm: acc.dosesIssuedFarm + row.dosesIssuedFarm,
        dosesIssuedGadvasu: acc.dosesIssuedGadvasu + row.dosesIssuedGadvasu,
        dosesUsedExperiments: acc.dosesUsedExperiments + row.dosesUsedExperiments,
        closingBalance: acc.closingBalance + row.closingBalance,
      }), { openingBalance: 0, dosesProduced: 0, dosesSold: 0, dosesIssuedFarm: 0, dosesIssuedGadvasu: 0, dosesUsedExperiments: 0, closingBalance: 0 });
      autoTable(doc, {
        startY: 42,
        head: [["Bull", "Set", "Status", "Opening", "Produced", "Sold", "Farm Issue", "GADVASU Issue", "Experiment", "Closing"]],
        body: semenStockReportRows.length ? semenStockReportRows.map((row) => [
          row.bullTag, row.setNo, row.status, row.openingBalance, row.dosesProduced, row.dosesSold, row.dosesIssuedFarm, row.dosesIssuedGadvasu, row.dosesUsedExperiments, row.closingBalance,
        ]) : [["No records", "", "", "", "", "", "", "", "", ""]],
        foot: semenStockReportRows.length ? [["TOTAL", "", "", total.openingBalance, total.dosesProduced, total.dosesSold, total.dosesIssuedFarm, total.dosesIssuedGadvasu, total.dosesUsedExperiments, total.closingBalance]] : undefined,
        theme: "grid",
        headStyles: { fillColor: [16, 120, 83] },
        styles: { fontSize: 7, cellPadding: 1.5 },
        footStyles: { fillColor: [226, 247, 239], textColor: [15, 23, 42], fontStyle: "bold" },
      });
      doc.save(`DairyScope-semen-stock-${fyLabel}-${monthLabel.replace(/\s+/g, "-")}.pdf`);
      return;
    }
  ...
  function addSemenStockRecord() {
    patchSelected((a) => {
      const current = (a.maleDetails.semenStockRecords || []).map(cleanSemenStockRecord);
      const previousClosing = current.length ? calculateSemenClosingBalance(current[current.length - 1]) : "";
      return { ...a, maleDetails: { ...a.maleDetails, semenStockRecords: [...current, makeSemenStockRecord(previousClosing)] } };
    });
  }

  function removeSemenStockRecord() {
    patchSelected((a) => {
      const current = (a.maleDetails.semenStockRecords || []).map(cleanSemenStockRecord);
      if (current.length <= 1) return a;
      return { ...a, maleDetails: { ...a.maleDetails, semenStockRecords: current.slice(0, -1) } };
    });
  }

  function updateSemenStockRecord(rowIdx, key, value) {
    patchSelected((a) => {
      const current = (a.maleDetails.semenStockRecords || []).map(cleanSemenStockRecord);
      current[rowIdx] = { ...(current[rowIdx] || makeSemenStockRecord()), [key]: ["openingBalance", "dosesProduced", "dosesSold", "dosesIssuedFarm", "dosesIssuedGadvasu", "dosesUsedExperiments"].includes(key) ? parseNumericValue(value) : value };
      return { ...a, maleDetails: { ...a.maleDetails, semenStockRecords: current } };
    });
  }
              ...
              <div className="rounded-[28px] border border-white/10 bg-white/[0.04] p-4">
                <div className="flex items-center gap-3">
                  <div className="flex h-16 w-16 items-center justify-center overflow-hidden rounded-full border border-emerald-300/30 bg-white p-2 shadow-[0_12px_28px_rgba(16,185,129,0.18)]">
                    <img src={DairyScopeLogo} alt="DairyScope logo" className="h-full w-full object-contain" />
                  </div>
                  <div>
              ...
              <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                <div className="flex items-center gap-4">
                  <div className="hidden h-12 w-12 items-center justify-center overflow-hidden rounded-full border border-emerald-100 bg-white p-1 shadow-sm sm:flex">
                    <img src={DairyScopeLogo} alt="DairyScope" className="h-full w-full object-contain" />
                  </div>
                  <div>
          ...
          <Section title="Reports">
            <div className="space-y-6">
              ...
              <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
                ...
                <button className={`rounded-[26px] border p-5 text-left shadow-[0_12px_28px_rgba(15,23,42,0.06)] transition hover:-translate-y-1 hover:shadow-lg ${activeReportPanel === "semenStock" ? "border-emerald-500 bg-emerald-50" : "border-slate-200/70 bg-white/95"}`} onClick={() => setActiveReportPanel("semenStock")}>
                  <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-emerald-50 text-xl">🧪</div>
                  <div className="mt-4 text-lg font-bold text-slate-950">Semen Stock Report</div>
                  <p className="mt-2 text-sm leading-6 text-slate-600">Export bull-wise semen stock for selected month or whole financial year.</p>
                </button>
              ...
              {activeReportPanel === "semenStock" && (
                <div className="rounded-[28px] border border-emerald-100 bg-white/95 p-5 shadow-[0_12px_28px_rgba(15,23,42,0.06)]">
                  <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                    <div>
                      <div className="text-xl font-bold text-emerald-900">Semen Stock Report</div>
                      <div className="mt-1 text-sm text-slate-600">Select financial year and month to preview additive bull-wise semen stock totals on screen, then export the same report as PDF.</div>
                    </div>
                    <button className="rounded-xl bg-emerald-600 px-4 py-2 text-sm font-bold text-white hover:bg-emerald-700" onClick={() => exportDairyScopeReport("semenStock")}>Export PDF</button>
                  </div>
                  <div className="mb-4 grid gap-4 md:grid-cols-2">
                    ...
                    <SelectField label="Month" value={selectedSemenStockMonth} onChange={setSelectedSemenStockMonth} options={["Overall","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec","Jan","Feb","Mar"]} />
                  </div>
                    ...
                    <table className="w-full table-fixed text-xs">
                      <thead>
                        <tr className="bg-emerald-50 text-left text-emerald-900">
                          {["Bull", "Set", "Status", "Opening", "Produced", "Sold", "Farm issue", "GADVASU", "Experiment", "Closing"].map((h) => <th key={h} className="border border-emerald-100 px-2 py-2 align-top">{h}</th>)}
                        </tr>
                      </thead>
                      <tbody>
                        {semenStockReportRows.length ? semenStockReportRows.map((row, idx) => (
                          <tr key={`semen-report-${row.bullTag}-${idx}`} className="align-top">
                            <td className="border border-emerald-100 px-2 py-2 font-semibold">{row.bullTag || "—"}</td>
                            ...
                            <td className="border border-emerald-100 px-2 py-2 text-right">{row.openingBalance}</td>
                            <td className="border border-emerald-100 px-2 py-2 text-right">{row.dosesProduced}</td>
                            <td className="border border-emerald-100 px-2 py-2 text-right">{row.dosesSold}</td>
                            <td className="border border-emerald-100 px-2 py-2 text-right">{row.dosesIssuedFarm}</td>
                            <td className="border border-emerald-100 px-2 py-2 text-right">{row.dosesIssuedGadvasu}</td>
                            <td className="border border-emerald-100 px-2 py-2 text-right">{row.dosesUsedExperiments}</td>
                            <td className="border border-emerald-100 bg-emerald-50 px-2 py-2 text-right font-bold text-emerald-900">{row.closingBalance}</td>
                          </tr>
                        )) : (
                          <tr><td colSpan={10} className="border border-emerald-100 px-3 py-6 text-center text-slate-500">No semen stock records for the selected period.</td></tr>
                        )}
                      </tbody>
                      {semenStockReportRows.length > 0 && (
                        <tfoot>
                          <tr className="bg-emerald-50 font-bold text-emerald-950">
                            <td className="border border-emerald-100 px-2 py-2" colSpan={3}>Total</td>
                            <td className="border border-emerald-100 px-2 py-2 text-right">{semenStockReportRows.reduce((s, r) => s + r.openingBalance, 0)}</td>
                            <td className="border border-emerald-100 px-2 py-2 text-right">{semenStockReportRows.reduce((s, r) => s + r.dosesProduced, 0)}</td>
                            <td className="border border-emerald-100 px-2 py-2 text-right">{semenStockReportRows.reduce((s, r) => s + r.dosesSold, 0)}</td>
                            <td className="border border-emerald-100 px-2 py-2 text-right">{semenStockReportRows.reduce((s, r) => s + r.dosesIssuedFarm, 0)}</td>
                            <td className="border border-emerald-100 px-2 py-2 text-right">{semenStockReportRows.reduce((s, r) => s + r.dosesIssuedGadvasu, 0)}</td>
                            <td className="border border-emerald-100 px-2 py-2 text-right">{semenStockReportRows.reduce((s, r) => s + r.dosesUsedExperiments, 0)}</td>
                            <td className="border border-emerald-100 px-2 py-2 text-right">{semenStockReportRows.reduce((s, r) => s + r.closingBalance, 0)}</td>
                          </tr>
                        </tfoot>
                      )}
                    </table>
                ...
                <div className="mt-7 grid gap-5 lg:grid-cols-2">
                  <div className="rounded-[28px] border border-emerald-100 bg-emerald-50/60 p-5 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-lg">
                    <div className="flex items-start gap-5">
                      <img src={Highlight1} alt="Herd Intelligence" className="ml-3 h-28 w-40 flex-shrink-0 object-contain object-center" />
                      <div>
                  ...
                  <div className="rounded-[28px] border border-cyan-100 bg-cyan-50/60 p-5 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-lg">
                    <div className="flex items-start gap-5">
                      <img src={Highlight2} alt="Genetic and Breeding Engine" className="ml-3 h-28 w-40 flex-shrink-0 object-contain object-center" />
                      <div>
                  ...
                  <div className="rounded-[28px] border border-blue-100 bg-blue-50/60 p-5 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-lg">
                    <div className="flex items-start gap-5">
                      <img src={Highlight3} alt="Production Analytics" className="ml-3 h-28 w-40 flex-shrink-0 object-contain object-center" />
                      <div>
                  ...
                  <div className="rounded-[28px] border border-violet-100 bg-violet-50/60 p-5 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-lg">
                    <div className="flex items-start gap-5">
                      <img src={Highlight4} alt="Reproduction Monitoring" className="ml-3 h-28 w-40 flex-shrink-0 object-contain object-center" />
                      <div>
                  ...
                  <div className="rounded-[28px] border border-amber-100 bg-amber-50/60 p-5 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-lg">
                    <div className="flex items-start gap-5">
                      <img src={Highlight5} alt="Bull Evaluation System" className="ml-3 h-28 w-40 flex-shrink-0 object-contain object-center" />
                      <div>
                  ...
                  <div className="rounded-[28px] border border-slate-200 bg-slate-50/80 p-5 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-lg">
                    <div className="flex items-start gap-5">
                      <img src={Highlight6} alt="Animal Documentation" className="ml-3 h-28 w-40 flex-shrink-0 object-contain object-center" />
                      <div>
          ...
          <div className="grid gap-5">
            ...
            {uiSelectedAnimal?.category === "Female" && (
              <Section title="Female Animal Tabs">
                ...
                {detailTab === "semen stock" && (
                  <div className="space-y-5">
                    <div className="rounded-2xl border border-emerald-100 bg-white p-4 shadow-sm">
                      <div className="mb-2 flex flex-wrap items-center justify-between gap-3">
                        <div>
                          <div className="text-lg font-bold text-emerald-900">Semen Stock Register</div>
                          <div className="text-sm text-slate-600">Closing balance = Opening + Produced − Sold − Farm issue − GADVASU issue − Experimental use.</div>
                        </div>
                        <div className="flex gap-3"><button className="flex h-12 w-12 items-center justify-center rounded-2xl bg-emerald-600 text-3xl font-bold text-white hover:bg-emerald-700" onClick={addSemenStockRecord}>+</button><button className="flex h-12 w-12 items-center justify-center rounded-2xl border border-slate-300 bg-white text-3xl font-bold text-slate-700 hover:bg-slate-50" onClick={removeSemenStockRecord}>−</button></div>
                      </div>
                      <div className="space-y-4">
                        {(activeAnimal.maleDetails.semenStockRecords || []).map(cleanSemenStockRecord).map((rec, idx) => (
                          <div key={`semen-${idx}`} className="rounded-[22px] border border-emerald-100 bg-white/80 p-4">
                            <div className="mb-3 text-sm font-bold text-emerald-900">Stock entry {idx + 1}</div>
                            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                              <TextField label="Opening balance" value={rec.openingBalance || ""} onChange={(v) => updateSemenStockRecord(idx, "openingBalance", v)} />
                              <TextField label="Semen doses produced" value={rec.dosesProduced || ""} onChange={(v) => updateSemenStockRecord(idx, "dosesProduced", v)} />
                              <DateField label="Date of production" value={rec.productionDate || ""} onChange={(v) => updateSemenStockRecord(idx, "productionDate", v)} />
                              <TextField label="Semen doses sold" value={rec.dosesSold || ""} onChange={(v) => updateSemenStockRecord(idx, "dosesSold", v)} />
                              <DateField label="Date of sale" value={rec.saleDate || ""} onChange={(v) => updateSemenStockRecord(idx, "saleDate", v)} />
                              <TextField label="Issued to Farm" value={rec.dosesIssuedFarm || ""} onChange={(v) => updateSemenStockRecord(idx, "dosesIssuedFarm", v)} />
                              <DateField label="Farm issue date" value={rec.farmIssueDate || ""} onChange={(v) => updateSemenStockRecord(idx, "farmIssueDate", v)} />
                              <TextField label="Issued to GADVASU" value={rec.dosesIssuedGadvasu || ""} onChange={(v) => updateSemenStockRecord(idx, "dosesIssuedGadvasu", v)} />
                              <DateField label="GADVASU issue date" value={rec.gadvasuIssueDate || ""} onChange={(v) => updateSemenStockRecord(idx, "gadvasuIssueDate", v)} />
                              <TextField label="Used for experiments" value={rec.dosesUsedExperiments || ""} onChange={(v) => updateSemenStockRecord(idx, "dosesUsedExperiments", v)} />
                              <DateField label="Experiment date" value={rec.experimentDate || ""} onChange={(v) => updateSemenStockRecord(idx, "experimentDate", v)} />
                              <TextField label="Closing balance" value={String(calculateSemenClosingBalance(rec))} onChange={() => {}} readOnly />
                            </div>
                          </div>
                        ))}
                      </div>
